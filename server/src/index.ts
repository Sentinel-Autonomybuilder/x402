...
'Windows: zero prereqs. `connect({ mnemonic, subscriptionId, feeGranter, nodeAddress, protocol: "v2ray" })` auto-downloads the v2ray binary (userspace SOCKS5, ~70% of nodes, no admin). WireGuard (~30%, faster) needs admin once; the SDK gates on admin BEFORE payment, so no funds are wasted. See sentinel.connectWindows.',
    ...
    example: {
      ...
      code: [
        ...
        '',
        'await setup();                                // optional: pre-fetch V2Ray (no admin). connect() also auto-fetches it.',
        ...
        "await connect({ mnemonic: wallet.mnemonic, subscriptionId, feeGranter, nodeAddress, protocol: 'v2ray' });  // v2ray = zero admin, zero manual install (Windows)",
      ].join('\n'),
    },
    sentinel: {
      chain: 'sentinelhub-2',
      ...
      setup: {
        ...
        note: 'OPTIONAL on Windows — connect({ protocol: "v2ray" }) auto-downloads V2Ray on first use, so you can skip setup() entirely. If you do run it on a fresh machine: auto-downloads V2Ray to ~/.sentinel-sdk/bin (no admin) and, only when run elevated, auto-installs the WireGuard MSI (needs admin). Returns { os, node, v2ray, wireguard, admin }. v2ray ok => you can connect with zero manual installs and zero admin via protocol: "v2ray".',
      },
      walletCreate: {
        package: 'blue-js-sdk/ai-path',
        fn: 'createWallet()',
